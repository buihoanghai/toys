const crawlHrefLang = require('./crawlHrefLang');
...
async function checkStatusCodeArr(arr) {
	var result = [];
	for (var i = 1; i < arr.length; i++) {
		let url = arr[i][0];
		let item = _.clone(arr[i]);
		let statusCode = await getStatusCode(url);
		item.push(statusCode);
		result.push(item);
	}
	return result;
}

async function buildInternationalArr(arr) {
	arr = await checkStatusCodeArr(arr);
	var result = [];
	result.push(["Index", "Originating URL", "Crawl date", "Alternate URL", "Status Code", "Robot"]);
	try {
		for (var i = 0; i < arr.length; i++) {
			let statusCode = arr[i][3];
			let url = arr[i][0];
			let alternateUrl = arr[i][2];
			let item = _.clone(arr[i]);
			item.unshift(i);
			if (statusCode !== 404) {
				let data = await crawlHrefLang.crawl(url);
				console.log(i, data.robot);
				item.push(data.robot);
			}
			result.push(item);
		}
	} catch (e) {
